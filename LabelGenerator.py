...
if "--roboto" in sys.argv:
    ...
    for font_name in ["ArialBd.ttf", "Arial_Bold.ttf"]:
        ...
        print(
            "On Mac OS the '--roboto' flag is mandatory because this script currently"
        )
...
class StickerRect:
    def __init__(
        self, c: Canvas, layout: PaperConfig, row: int, column: int, mirror: bool
    ):
    ...
    def get_prefixed_number(self) -> str:
        if self.ohms_exp % 3 == 0:
            if self.ohms_val % 100 == 0:
                ...
                return (
                    str(self.ohms_val // 100) + "." + str((self.ohms_val % 100) // 10)
                )
            else:
...
def draw_resistor_stripe_border(
    c: Canvas, x: float, y: float, width: float, height: float
) -> None:
    c.setLineWidth(0.3)
...
def draw_resistor_stripe(
    c: Canvas, x: float, y: float, width: float, height: float, stripe_value: int
) -> None:
    if 0 <= stripe_value <= 9:
...
def draw_resistor_colorcode(
    ...
    c.linearGradient(
        x + width / 2, y + border + height, x + width / 2, y + border, (color1, color2)
    )
    c.restoreState()
...
def draw_resistor_sticker(
    ...
    with StickerRect(c, layout, row, column, mirror) as rect:
        ...
        draw_resistor_colorcode(
            c,
            resistor_value,
            toColor("hsl(55, 54%, 100%)"),
            toColor("hsl(55, 54%, 70%)"),
        ...
        draw_resistor_colorcode(
            c,
            resistor_value,
            toColor("hsl(197, 59%, 100%)"),
            toColor("hsl(197, 59%, 73%)"),
        ...
        c.drawString(
            rect.left + rect.width / 2 + rect.width / 32,
            rect.bottom + rect.height / 13,
            get_3digit_code(resistor_value),
        )
        c.drawCentredString(
            rect.left + rect.width * 3 / 4,
            rect.bottom + rect.height / 13,
            get_4digit_code(resistor_value),
        )
        c.drawRightString(
            rect.left + rect.width - rect.width / 32,
            rect.bottom + rect.height / 13,
            get_eia98_code(resistor_value),
        )
...
def render_stickers(
    ...
    def flatten(
        elem: Union[Optional[float], List[Optional[float]]],
    ) -> List[Optional[float]]:
    ...
    # Flatten
    values_flat: List[Optional[float]] = [
        elem for nested in values for elem in flatten(nested)
    ]
    ...
    for position, value in enumerate(values_flat):
        rowId = (
            position // layout.num_stickers_horizontal
        ) % layout.num_stickers_vertical
        ...
        if value is not None:
            draw_resistor_sticker(
                c, layout, rowId, columnId, value, draw_center_line, False
            )
            if draw_both_sides:
    ...
    for row in range(layout.num_stickers_vertical):
        for column in range(layout.num_stickers_horizontal):
            with StickerRect(c, layout, row, column, False) as rect:
                ...
                c.setLineWidth(0)
                c.roundRect(
                    rect.left, rect.bottom, rect.width, rect.height, rect.corner
                )
...
def main() -> None:
    ...
    # Render the stickers
    render_stickers(
        c, layout, resistor_values, draw_outlines, draw_center_line, draw_both_sides
    )
